...
If you installed via the one-line installer, this runs as `skelo` on your
PATH. Running straight from a cloned repo? Use `./skelo.sh` the same way.

USAGE
  skelo <command> [args...]        (or: ./skelo.sh <command> [args...])
